...
87. **S8 do E2: o portão, e o não-envio que se vê.** O Judge 1 pré-envio julga contra as **rubricas do S1** — o portão que bloqueia um envio e o portão que ativa uma versão falam os mesmos critérios.
    **Regra por severidade, fixada com o Bruno (2026-08-03):** critério `critical` reprovado → **nunca envia e nem regenera** (a segunda tentativa de uma violação de segurança é outra violação de segurança); só `standard` reprovado → regenera, **com os critérios falhos como feedback** (regenerar sem dizer o que estava errado é repetir); regenerações esgotadas com falha só standard → **envia a melhor versão** (o cliente nunca fica no vácuo por deslize de tom); juiz ilegível conta como reprovação (fail-closed) e, se **nenhuma** tentativa produziu julgamento utilizável, não sai nada — "não julgado" jamais vira "aprovado" por esgotamento.
    **O piso 0,85 do D3 NÃO se aplica por mensagem.** Ele é do gate de ativação, sobre um pacote (S3/S11); aplicá-lo por mensagem seria o mesmo número com dois significados — exatamente o erro que a decisão 82(a) existe para impedir.
    **O ramo de não-envio no SQL.** `internal.conclude_turn` ganhou, por `create or replace` com a **mesma assinatura** (aditivo, N-1 compatível, com teste de regressão do caminho antigo): conteúdo nulo → faz o CAS, avança `last_processed_seq`, solta a lease, incrementa `version` e **não** insere na outbox nem na `messages`. Sem isso a conversa ficaria presa e o coalescer recriaria o job para sempre — a resposta reprovada viraria moedor de token em vez de alerta. **Duas formas de "nada a dizer" são recusadas**, não uma: `Jsonb(None)` do psycopg renderiza o valor JSON `null`, não SQL NULL, e um chamador que esquecesse disso enfileiraria uma mensagem com payload `null` para o sender encontrar no pior momento.
    **O silêncio é observável por construção:** o alerta é gravado **antes** da conclusão, então uma morte no meio deixa alerta duplicado (benigno e visível) em vez de silêncio que ninguém vê. É a resposta direta ao aviso que mora dentro da própria função ("conclusão sem linha na outbox é o cliente nunca receber e nada aparecer em lugar nenhum") — e o aviso ficou no corpo, ao lado do ramo.
    **As sabotagens, com raio medido.** A coroa (tirar o Judge do caminho) derrubou **10**: as **duas** metades do cenário `pipeline` — inclusive a positiva, porque resposta aprovada também precisa deixar nota (RNF-050) — e os 8 do laço. A do banco (o ramo nulo enfileirando assim mesmo) derrubou **3**: os dois casos `db` e o cenário-coroa, com a metade positiva intacta.
    **Achado de método, registrado para não repetir:** rodar os níveis `db` e `pipeline` **ao mesmo tempo** contra o mesmo banco é auto-sabotagem — o `pipeline` limpa a base inteira entre testes (`clean_slate`), e a colisão produziu 18 falhas que sumiram sozinhas na execução em série (827 e 31 verdes). No CI eles já são jobs distintos; localmente, um de cada vez.
    **Pendência assumida para o S9:** a gravação do score e do alerta hoje mora no responder de teste (`tests/support/judged.py`). O S9 tem de trazê-la para o responder real, senão "toda resposta registra o score do Judge 1" continua sendo boa intenção do chamador em vez de estrutura.

88. **S9a do E2: o agente real dentro do motor — e três coisas que só apareceram quando ele nasceu.** `build_responder` é a costura do D5: devolve a forma que o worker conhece (`respond(job)`) e, por dentro, carrega em transação curta (versão ativa, política do tenant, conversa, janela pendente, histórico), mede o think-gate, busca conhecimento, compõe as camadas do RF-010, chama o modelo medido e passa pelo Judge 1. **A pendência do S8 fechou aqui:** a nota de cada tentativa e o alerta do não-envio são gravados pelo próprio responder — deixar isso com o chamador era boa intenção, não estrutura.
    (a) **Recuperação determinística em vez de tool escolhida pelo modelo, no E2.** As duas tools do marco são **contexto incondicional**, e uma ida e volta a mais custa segundos numa conversa de WhatsApp. Tool que o modelo ESCOLHE chega no E3, quando existir escolha a fazer (pedido por id, rastreio, chamar humano). O rastro em `tool_calls` é o mesmo: quem executa continua sendo o `run_tool` do S7, com transação curta própria e validação de tenant.
    (b) **`get_customer_context` não tem consumidor no E2**, declarado em vez de escondido: a camada `customer_context` do RF-010 fala de PEDIDOS, e o espelho de pedidos chega no E3. Inventar "cliente sem histórico" para quem já conversou três vezes seria pior que a ausência (decisão 86d).
    (c) **Conta sem versão ativa levanta exceção**, nunca conclui em silêncio: concluir marcaria a mensagem como respondida e o cliente esperaria para sempre sem ninguém saber; a exceção é a escada até a DLQ, onde um humano olha. Janela vazia, ao contrário, conclui sem enviar — é o caso em que não há o que responder.
    **Achado nº 1 — a trava de fronteira estava certa e eu estava errado.** `agent_core.responder` reprovou o contrato "nada chama a API do WhatsApp exceto os senders" por um caminho que ninguém tinha visto: `repository.engine` importa `channels.port` (a outbox reivindicada é um `ClaimedSend`), então **qualquer** módulo que importasse `engine` só para pegar `scope_to_tenant` passava a alcançar `channels` por tabela. Conserto: nasceu `repository/scope.py` com a função, `engine` a reexporta (nenhum chamador do E1 mudou) e as tools do S7 passaram a importar de lá. Afrouxar o contrato teria escondido a segunda linha de defesa por conveniência.
    **Achado nº 2 — a imagem não levava as rubricas.** O Dockerfile copiava só `src/`, e o responder lê as rubricas do Judge 1 de `evals/`. O container subiria e morreria na primeira mensagem, **em produção, não no CI**. Conserto: `COPY evals/` nas duas etapas + teste de fitness que lê o Dockerfile e reprova se a linha sumir. O caminho é derivado do pacote instalado (`parents[2]/evals/rubrics`), não do cwd, porque o processo sobe dentro de um container.
    **Achado nº 3, de método — um teste de `pipeline` que passava sabotado.** A guarda "conta sem versão ativa" esperava `read_ct >= 1` na fila; esse sinal é verdadeiro por um instante **nos dois desfechos**, então a sabotagem (concluir em silêncio) passou. A propriedade é do responder, não do motor: virou chamada direta em `tests/db/test_responder_guards.py`, onde há um desfecho por chamada, e lá a mesma sabotagem reprova com `DID NOT RAISE`. **Regra que fica: sinal que é verdadeiro nos dois desfechos não é asserção, é corrida.**
    **Anotado, não resolvido:** o responder abre **uma conexão por job**. Precedente do E1 (o canal fake e o responder segurável fazem igual) e simples, mas é o candidato natural a um pool quando o volume justificar — antes do piloto do E7.

89. **Achado de revisão: o processo podia responder cliente sem passar pelo Judge 1 — e nada falhava.** Não é defeito do S9a; é herança do E1 virando furo no PR que põe o agente real em produção. `_factory_from_env` devolvia `None` com a variável ausente e `app.run` cai em `respond = respond or fixed_responder()`, então **`AGENTS_RESPONDER` esquecida no compose da VPS = runtime saudável respondendo todo mundo com a frase constante, sem `judge_scores` e sem alerta**. Contraria o invariante que o `CLAUDE.md` escreve sem exceção ("no exceptions, including load tests"). A **forma** da falha é o que a torna grave: não quebra — envia. Comparar com o Dockerfile sem `evals/` do próprio S9a, que **morria** na primeira mensagem; morrer é o comportamento bom.
    **Conserto:** morte alta no entrypoint quando `AGENTS_RESPONDER` falta ou vem em branco, no molde do token do canal (decisão 67). `app.run` **mantém** o default, porque os cenários do motor dependem dele e a Lei 1 proíbe tocá-los — o que deixou de existir é o caminho de PRODUÇÃO até ele. `AGENTS_CHANNEL` fica de fora de propósito, e um teste fixa a assimetria: sem canal nada é enviado, então ausente ali é seguro.
