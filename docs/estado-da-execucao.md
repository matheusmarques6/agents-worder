...
55. **O selector loop agora é forçado no entrypoint** (`__main__`, só em win32): o subprocesso subia com Proactor e o psycopg async recusa. É a decisão 24 reaparecendo uma camada abaixo — o conftest protegia os testes em-processo, mas o processo real também precisa. Em Linux (produção e CI) a linha não muda nada.
