...
- **RF-032** Antes de processar evento atrasado (> 5 min), o sistema verifica staleness: mensagem mais recente na conversa, pedido pago nesse meio-tempo ou contato em supressão abortam o disparo.
  **Esclarecimento (E3 · S1, decisão D12):** a escada de proteção verifica staleness **sempre**, não só acima de 5 minutos. A idade descreve *quando a verificação passou a ser necessária* (drenagem de fila após uma queda), não licença para pular proteção enquanto o evento está fresco — e desde a decisão D11 o primeiro toque de todo funil nasce como `scheduled_touch` já vencido, ou seja, **o caminho fresco virou o caminho normal**. Ser mais estrito que a letra do requisito só pode suprimir um envio, nunca criar um: um contato que escreveu há 30 segundos está numa conversa que o agente já está respondendo por reação, e um toque de funil por cima é exatamente o que irrita quem se queria recuperar.
