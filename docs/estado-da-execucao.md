...
55. **O selector loop agora é forçado no entrypoint** (\, só em win32): o subprocesso subia com Proactor e o psycopg async recusa. É a decisão 24 reaparecendo uma camada abaixo — o conftest protegia os testes em-processo, mas o processo real também precisa. Em Linux (produção e CI) a linha não muda nada.
56. **O harness de kill usa \, não o subprocesso do asyncio**: selector loop não gerencia filhos no Windows. Espera é sempre por predicado contra o banco — o banco é o observável que o processo e o teste compartilham, e é o mesmo que o monitoramento de produção vai usar. \ no Windows é TerminateProcess (não existe sinal limpo), então o smoke do desligamento gracioso roda só no CI Linux, com skip explícito.
57. **O sender só existe quando existe canal.** \ é uma fábrica \; ausente, a task do sender nem sobe — explícito, em vez de um sender inventando desfechos contra um canal que não existe. E o FakeChannel grava o envio ANTES de retornar, espelhando o mundo real: o provedor tem a mensagem no momento em que a API aceita, aconteça o que acontecer com o nosso processo depois — é a ordem de que o cenário 10 depende.
