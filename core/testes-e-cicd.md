...
| `dispatch` | supressão pelos 3 motivos; limites de proteção (1 proativo/contato/24h somando origens, 4 toques por funil, intervalo mínimo de 72h entre funis, mensagem reativa nunca bloqueada); decisão de obsolescência de evento (mensagem posterior / pedido pago / contato suprimido); cálculo de cadência do funil; seleção de canal (`cloud \| evolution \| auto`) |
...
11. Dispatch: `order_paid` cancela os `scheduled_touches` pendentes; supressão bloqueia o proativo; evento obsoleto é descartado; o funil completo respeita 4 toques, cadência e intervalo entre funis.
