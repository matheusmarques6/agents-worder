...
- [ ] S1 rubricas + pack (100% sintético, ADR-12) → S2 migration da fatia → S3 harness de evals → S4 prompt em camadas → S5 porta LLM + trava de rede por fitness → S6 conhecimento → S7 tools autovalidantes → S8 Judge 1 pré-envio (sabotagem-coroa) → S9 responder real + shadow + pós-envio → S10 T4 condicional → S11 pack contra LLM real → S12 provas
